...
class HTTP_StaticMerger
{
     ...
     */
     //$secret, $documentRoot = null, $baseUri = '/', $noCompression = false
    ...
    {   
        ...
        if (!$this->getOption('documentRoot', false)) {
            $this->setOption('documentRoot', $_SERVER['DOCUMENT_ROOT']);
        }
    ...
    public function main($charset = null)
    {
        ...
        $charset = $this->getOption('charset', 'UTF-8');
     ...
     * @return string
     */
    public function getHtml($uriList, $handler = null,  $noMerge = null, $additionalAttr = null)
    {
        if (!is_null($noMerge)) {
            $this->setOption('noMerge', $noMerge);
        }
        
        if (!is_null($additionalAttr)) {
            $this->setOption('additionalAttr', $additionalAttr);
        }
        ...
        if ($this->getOption('noMerge', false)) {
            
            $uris = array();
            foreach ($uriList as $uri) {
                $uri = $this->_rel2abs($uri, $this->getOption('handler'));
                $uri .= (false === strpos($uri, "?")? '?' : "&") . filemtime($this->_uri2fname($uri));
                if ($this->getOption('handler') && preg_match('{^((\w+:)?//[^/]+)}s', $this->getOption('handler'), $m)) {
            ...
            foreach ($uriList as $i => $uri) {
              ...
              $uris = array_merge(
                  array($this->_listToRequest($this->getOption('handler'), array_values($uriList))), 
                  $absUrls
              );
        }
        // Build tags.
        ...
        foreach ($uris as $uri) {
            if ($mime == 'text/css') {
                $tags[] = '<link rel="stylesheet" type="text/css" href="' . htmlspecialchars($uri) . '"' . ($this->getOption('additionalAttr', '') ? ' ' . $this->getOption('additionalAttr', '') : '') . ' />';
            } else {
                // &#160; is for XHTML.
    ...
    {
        if (!function_exists('gzdeflate') || $this->getOption('noCompression', false)) {
            return $s;
        }
    ...
    {
        return md5($this->getOption('secret') . $this->_getHandlerPath($handler) . $compressed);
    }
    ...
    {
        if (substr($uri, 0, 1) != '/') {
            $base = $this->getOption('baseUri', '/');
            $uri = $base . (substr($base, -1) == '/'? '' : '/') . $uri;
        }
        return $uri;
    }
    ...
    private function _uri2fname($uri)
    {
        ...
        $fname = $this->getOption('documentRoot') . (substr($uri, 0, 1) == '/'? '' : '/') . $uri;
        $realpath = @realpath($fname);
        if (!$realpath || substr($realpath, 0, strlen($this->getOption('documentRoot'))) !== $this->getOption('documentRoot')) {
            return null;
        }
        return $realpath;
    }
    ...
    private function _processCss($data, $uri)
    {
        ...
        // ������������ URL-� � ���������
        $this->_pagePath = pathinfo($uri, PATHINFO_DIRNAME);

        // ������������ ����������� ���� ": __ url(...)"
        $data = preg_replace_callback('#
                \: .*? url \s* \( \s* (["\']?) ([^"\'()]+) \1 \s* \)
        #sxi', array($this, '_processImagesPath'), $data);
        
        
        if (false === strpos($data, "@import")) {
            return $data;
        }
        $prevBase = $this->getOption('baseUri', '/');
        
        $this->setOption('baseUri', preg_replace('{[^/]+$}s', '', $this->_rel2abs($uri)));
        ...
        $this->setOption('baseUri', $prevBase);
        return $result;
    }
    ...
    public function _processImagesPath($m) {
        ...
        if (in_array(strtok($m[2], ':'), array('http', 'https'))) return $m[0];

        return str_replace($m[2], $this->_pagePath . '/' . $m[2], $m[0]);
    }
}
